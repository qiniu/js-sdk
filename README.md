...
1. 进入项目根目录，执行 `npm install` 安装依赖库，然后执行 `npm run serve && npm run build:dev` 运行服务 demo1,或者直接 `make all` 运行 demo1； demo2 为测试es6语法的 demo，进入 demo2 目录，执行 `npm install`，然后 `npm start` 运行 demo2，demo1 和 demo2 都共用一个 server 服务
