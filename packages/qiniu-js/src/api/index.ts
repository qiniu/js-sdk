import type { Config, UploadInfo, UpProtocol } from '../upload/base'
 ...
 * @param  {UpProtocol} protocol
 * @returns Promise
 */
export async function getUpHosts(accessKey: string, bucketName: string, protocol: UpProtocol): Promise<UpHosts> {
  const params = `ak=${encodeURIComponent(accessKey)}&bucket=${encodeURIComponent(bucketName)}`
...
export type UploadCompleteData<T = any> = T
